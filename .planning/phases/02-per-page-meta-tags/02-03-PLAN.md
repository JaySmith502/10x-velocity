...
must_haves:
  truths:
    ...
    - "CSS+JS fix targets chat widget H1 (light DOM verified; shadow DOM requires post-deploy check)"
    - "Each page has exactly one H1 tag when the chat widget renders in light DOM"
  artifacts:
...
<task type="auto">
  ...
  <verify>
    Run `npm run build` -- build must succeed.
    Grep src/index.css for `.chat-layout .chat-header h1` -- should find the CSS rule.
    Grep src/components/layout/RootLayout.tsx for `useChatWidgetH1Fix` -- should find both the hook definition and invocation.
    Grep src/components/layout/RootLayout.tsx for `MutationObserver` -- should find the observer setup.
    Grep src/components/layout/RootLayout.tsx for `querySelector.*chat-header` -- should find the DOM query.
  </verify>
  <done>CSS fallback rule added to index.css. MutationObserver hook in RootLayout.tsx watches for and replaces the chat widget H1 with a span. Build succeeds. Hook is silently inert when widget is absent (dev environment). Post-deploy verification needed to confirm light DOM vs shadow DOM behavior on live site.</done>
</task>
...
6. All verification via grep -- no manual dev server check needed (widget not present in dev)
</verification>
...
- Post-deploy check needed to verify fix works on live site (shadow DOM uncertainty)
</success_criteria>
