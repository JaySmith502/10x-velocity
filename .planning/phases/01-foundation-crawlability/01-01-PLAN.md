...
3. **Create `public/sitemap.xml`** listing all routes from `src/App.tsx`. Use `https://www.10xvelocity.ai` as the base URL. Include every route EXCEPT the catch-all `*` (404). Assign priorities as follows:
   ...
   The complete route list (34 routes total):
   ```
   /
   /about
   /services
  ...
  <verify>
    ...
    - Count the `<url>` entries in sitemap.xml -- should be 34
  </verify>
  <done>
    ...
    - sitemap.xml exists in public/ with all 34 indexable routes, valid XML structure, and correct priorities
