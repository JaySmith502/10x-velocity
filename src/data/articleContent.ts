...
<h2>What do the 10x Velocity Amplify Meetings plans include?</h2>
<table>
...
<p>Only one Louisville provider publishes prices, so here is that answer: a single 2-hour Amplify Meeting with 10x Velocity is $999, and standing cadences run $2,500 per 4 weeks (biweekly), $5,000 (weekly), and $10,000 (twice weekly), with larger automation projects scoped and built inside the engagement. For every other local option, budget for a sales conversation before you get a number. MSP-delivered AI work is typically bundled into managed-services contracts; national consultancies price by proposal. Whatever you're quoted, apply the same test: is the price attached to a named workflow with a measurable baseline?</p>
...
<p>Run your own first — the rubric above takes 20 minutes per workflow and is free. A paid review earns its fee in two situations: when scores cluster at 2–3 and you can't tell which gap is load-bearing, or when departments disagree about where the hours actually go. That's what a single <a href="/packages">Amplify Meeting</a> is for ($999): two hours in which an AI strategist and an automation architect map your workflows with you and name the priority impact areas. The industry default for this deliverable is a sales call and an unpublished price; we'd rather you self-score, then book a session only if the answer isn't obvious.</p>
